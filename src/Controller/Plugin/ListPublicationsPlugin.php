...
namespace Publications\Controller\Plugin;
 ...
 * $plugin = $this->ListPublicationsPlugin();
 * $pluginView = $plugin->render();
 *
 * How to call this plugin with custom parameters:
 * $plugin = $this->ListPublicationsPlugin();
 * $parameters = array(
 *      'template_path' => 'MySiteTest/news/listpublications'
 * );
 * $pluginView = $plugin->render($parameters);
 *
 * How to add to your controller's view:
 * $view->addChild($pluginView, 'listpublications');
 *
 * How to display in your controller's view:
 * echo $this->listpublications;
 *
 *
 */
class ListPublicationsPlugin extends MelisTemplatingPlugin
{
    public function __construct($updatesPluginConfig = array())
    {
        $this->configPluginKey = 'publications';
        $this->pluginXmlDbKey = 'PublicationsList';
    ...
    public function front()
    {
        ...
        // Properties
//        $pageIdDetailPublication   = !empty($data['pageIdPublication']) ? $data['pageIdPublication'] : null;
        $siteId             = !empty($data['site_id'])    ? $data['site_id']    : null;

        // Filters
        $status             = true;
        $unpublishFilter    = true;
        $orderColumn        = !empty($data['column'])              ? $data['column']             : 'publication_date';
...
//        if (is_null($pageIdDetailPublication))
//        {
//            $pageIdDetailPublication = $pageId;
//        }
        ...
        // Retrieving News list using PublicationsService
        $publicationsSrv = $this->getServiceLocator()->get('PublicationsService');
        $publicationsList = $publicationsSrv->getPublicationsList($status, $langId, null, null, $dateMin, $dateMax, $unpublishFilter, null, null, $orderColumn, $order, $siteId ,$search);

        $listPublications = array();
        foreach ($publicationsList As $key => $val)
        {

            // Generate link to news
//            $link = $pageTreeService->getPageLink($pageIdDetailPublication, false);
//            $val['publicationLink'] = $link . '?publicationId=' . $val['publication_id'];

            // date formated of news
            $val['publicationDateFormated'] = date('d M Y',strtotime($val['publication_creation_date']));

            // Adding the News Data to result variable
            array_push($listPublications, $val);
        }
        ...
        if (!empty($formConfig)) {
            foreach ($formConfig as $formKey => $config) {
                ...
                if (!isset($parameters['validate'])) {
                    ...
                    if ($formKey == 'publications_list_plugin_filter_form')
                    {
                        if (!empty($post['date_min']) && !empty($post['date_max']))
                        {
                            if ($post['date_min'] > $post['date_max'])
                            {
                                $errors['date_max'] = array(
                                    'label' => $translator->translate('tr_publications_plugin_filter_date_range_to'),
                                    'inValidDates' => $translator->translate('tr_publications_plugin_invalid_dates'),
                                );
                            }
                        }
                    }
    ...
    public function loadDbXmlToPluginConfig()
    {
        ...
        if ($xml)
        {
            ...
            if (!empty($xml->pageIdPublication))
                $configValues['pageIdPublication'] = (string)$xml->pageIdPublication;
    ...
    public function savePluginConfigToXml($parameters)
    {
        ...
        if (!empty($parameters['pageIdPublication']))
            $xmlValueFormatted .= "\t\t" . '<pageIdPublication><![CDATA[' . $parameters['pageIdPublication'] . ']]></pageIdPublication>';
